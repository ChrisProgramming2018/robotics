...
class RoverState():
    def __init__(self):
        ...
        self.pos = (1,1) # Current position (x, y)
        self.pos_old = (0,0) # save old pos for checking if stucked
        ...
        self.rock_ang = 0
        self.rock_dist = 0
        self.stop_mode_count = 0 
        self.stop_to_turn = True
        self.found = False      # if false no new rock was found
        self.rock_pos = None
...
def telemetry(sid, data):
    ...
    if (time.time() - second_counter) > 1:
        ...
        frame_counter = 0
        second_counter = time.time()
    # print("Current FPS: {}".format(fps), end='\r')
    ...
    if data:
        ...
        if np.isfinite(Rover.vel):
            ...
            Rover = decision_step(Rover, data)
            if frame_counter % 10000 == 0 and False:
                print("data")
                print('steering_angle', data['steering_angle'])
                print('position', data['position'])
                print('throttle', data['throttle'])
                print('brake', data['brake'])
                print('speed', data['speed'])
                print('near sample', data['near_sample'])

            if frame_counter % 1000 == 0:
                print("Rover mode: {}  pos: {}  rock {}  brake: {}  throttle: {} steering: {} ".format(Rover.mode,
                    Rover.pos , Rover.rock_pos, Rover.brake, Rover.throttle,
                    Rover.steer), end="\r")
                #print("Found rock" ,Rover.found)
                #print("Brake" ,Rover.brake)
                #print("Throttel" ,Rover.throttle)
